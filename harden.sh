...
if ! ps -p $$ | grep -si bash; then
  echo "[!] Run with bash! (sh/bash harden.sh)"
  exit 1
fi
...
function main {
  ...
  if grep -s "AUTOFILL='Y'" ./harden.cfg; then
    ...
    sed -i "s/FW_ADMIN='/FW_ADMIN='$ADMINIP /" ./harden.cfg
    sed -i "s/SSH_GRPS='/SSH_GRPS='$(id "$($WBIN -ih | awk '{print $1}' | head -n1)" -ng) /" ./harden.cfg
    sed -i "s/CHANGEME=''/CHANGEME='$(date +%s)'/" ./harden.cfg
    sed -i "s/VERBOSE='N'/VERBOSE='Y'/" ./harden.cfg
  fi
